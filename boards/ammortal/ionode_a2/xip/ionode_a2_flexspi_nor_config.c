...
#include "ionode_a2_flexspi_nor_config.h"
...
const flexspi_nor_config_t qspi_flash_config = {
	.mem_config = {
		...
		.config_cmd_args[0] = (FLASH_DUMMY_VALUE << 4),
		.lookup_table = {
			/* Read LUTs */
			[0] = FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD, 0xEC,
				RADDR_SDR, FLEXSPI_4PAD, 0x20),
			[1] = FLEXSPI_LUT_SEQ(DUMMY_SDR, FLEXSPI_4PAD,
					FLASH_DUMMY_CYCLES, READ_SDR,
					FLEXSPI_4PAD, 0x04),
			...
			[4 * 9 + 0] = FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD,
				0x12, RADDR_SDR, FLEXSPI_1PAD, 0x20),
