...
MEMORY
    ...
    OCRAM ( rw ) : ORIGIN = (538968064), LENGTH = (262144) ITCM ( rw ) : ORIGIN = (0), LENGTH = (262144) DTCM ( rw ) : ORIGIN = (536870912), LENGTH = (262144) OCRAM1 ( rw ) : ORIGIN = (539230208), LENGTH = (524288) OCRAM2 ( rw ) : ORIGIN = (539754496), LENGTH = (524288)
...
KEEP(*(.boot_hdr.conf))
. += 0x1000 - (. - __rom_start_address);
KEEP(*(.boot_hdr.ivt))
KEEP(*(.boot_hdr.data))
 . += 0x1040 - (. - __rom_start_address);
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    OCRAM (NOLOAD) : { __OCRAM_start = .; KEEP(*(OCRAM)) KEEP(*(OCRAM.*)) __OCRAM_end = .; } > OCRAM __OCRAM_size = __OCRAM_end - __OCRAM_start; __OCRAM_load_start = LOADADDR(OCRAM); ITCM (NOLOAD) : { __ITCM_start = .; KEEP(*(ITCM)) KEEP(*(ITCM.*)) __ITCM_end = .; } > ITCM __ITCM_size = __ITCM_end - __ITCM_start; __ITCM_load_start = LOADADDR(ITCM); DTCM (NOLOAD) : { __DTCM_start = .; KEEP(*(DTCM)) KEEP(*(DTCM.*)) __DTCM_end = .; } > DTCM __DTCM_size = __DTCM_end - __DTCM_start; __DTCM_load_start = LOADADDR(DTCM); OCRAM1 (NOLOAD) : { __OCRAM1_start = .; KEEP(*(OCRAM1)) KEEP(*(OCRAM1.*)) __OCRAM1_end = .; } > OCRAM1 __OCRAM1_size = __OCRAM1_end - __OCRAM1_start; __OCRAM1_load_start = LOADADDR(OCRAM1); OCRAM2 (NOLOAD) : { __OCRAM2_start = .; KEEP(*(OCRAM2)) KEEP(*(OCRAM2.*)) __OCRAM2_end = .; } > OCRAM2 __OCRAM2_size = __OCRAM2_end - __OCRAM2_start; __OCRAM2_load_start = LOADADDR(OCRAM2);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
